...
class VienmandateSpider(CrawlSpider):
        name = "vienmandate1996_2"
        allowed_domains = ["www3.lrs.lt"]
	start_urls = ["http://www3.lrs.lt/n/rinkimai/seim96/rapgs2l.htm"]
	rules =[Rule(SgmlLinkExtractor(allow=['rapgpl.htm'],deny=['rezultatai/rezv_e_','_apg_e_']),'parse_apygarda',
		#'parse_apygarda', 
		 follow=True),
		...
		viso = int(hxs.select('//p/b/text()').extract()[0].encode())
		viso_biuleteniu = int(hxs.select('//p/b/text()').extract()[5]) 
	...
	def parse_apylinke(self,response):
		...
		viso_biuleteniu = int(hxs.select('//b/text()')[5].extract().encode('UTF8'))+int(hxs.select('//b/text()')[7].extract().encode('UTF8'))
		kandidatai = hxs.select('//table/tr')
