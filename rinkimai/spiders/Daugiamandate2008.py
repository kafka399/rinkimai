...
class DaugiamandateSpider(CrawlSpider):
        name = "daugiamandate2008"
        allowed_domains = ["www.vrk.lt"]
	start_urls = ["http://www.vrk.lt/2008_seimo_rinkimai/output_lt/rezultatai_daugiamand_apygardose/rezultatai_daugiamand_apygardose1turas.html"]
	rules =[Rule(SgmlLinkExtractor(allow=['output_lt/rezultatai_daugiamand_apygardose/apylinkes'],deny=['output_en']), 'parse_apylinke', follow=False),
		...
		for part in partijos:
			...
			if len(part.select('td/a/text()').extract())>0:
				...
				item['apylinkese'] =  (part.select('td/text()').extract()[3]).encode('UTF8')
				item['pastu'] = (part.select('td/text()').extract()[4]).encode('UTF8')
				item['nuo_galiojanciu_biuleteniu'] = (part.select('td')[6].select('text()')[0].extract()).encode('UTF8')#(part.select('td/text()').extract()[6]).encode('UTF8')
				yield item
