...
class Vienmandate2012Spider(CrawlSpider):
        name = "vienmandate2012_2"
        allowed_domains = ["www.vrk.lt"]
	start_urls = ["http://www.vrk.lt/2012_seimo_rinkimai/output_lt/rezultatai_vienmand_apygardose2/rezultatai_vienmand_apygardose2turas.html"]
	rules =[Rule(SgmlLinkExtractor(allow=['rezultatai_vienmand_apygardose2/rezultatai_vienmanate_apygarda'],deny=['output_en']), 'parse_apygarda', follow=True),
		Rule(SgmlLinkExtractor(allow=['rezultatai_vienmand_apygardose2/rezultatai_apylinke'],deny=['output_en']), 'parse_apygarda_rez', follow=False)
